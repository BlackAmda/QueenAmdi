/*
░██████╗░██╗░░░██╗███████╗███████╗███╗░░██╗
██╔═══██╗██║░░░██║██╔════╝██╔════╝████╗░██║
██║██╗██║██║░░░██║█████╗░░█████╗░░██╔██╗██║
╚██████╔╝██║░░░██║██╔══╝░░██╔══╝░░██║╚████║
░╚═██╔═╝░╚██████╔╝███████╗███████╗██║░╚███║
░░░╚═╝░░░░╚═════╝░╚══════╝╚══════╝╚═╝░░╚══╝
░█████╗░███╗░░░███╗██████╗░██╗
██╔══██╗████╗░████║██╔══██╗██║
███████║██╔████╔██║██║░░██║██║
██╔══██║██║╚██╔╝██║██║░░██║██║ █▀█ █▀▀█ █▀█ ▄█─ 
██║░░██║██║░╚═╝░██║██████╔╝██║ ─▄▀ █▄▀█ ─▄▀ ─█─ 
╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═════╝░╚═╝ █▄▄ █▄▄█ █▄▄ ▄█▄
Copyright (C) 2021 Black Amda.
Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.
*/

const QueenAmdi = require('queenamdi-public');
const Amdi = QueenAmdi.events
const Build = QueenAmdi.build

const {MessageType,Mimetype} = require('@blackamda/queenamdi-web-api');
const axios = require('axios')
const fetch = require('node-fetch') 
const cheerio = require('cheerio')
const exec = require('child_process').exec;
const yts = require( 'yt-search' )
const Language = require('../language');
const Lang = Language.getString('dl-video');
let LOL = Build.WORKTYPE == 'public' ? false : true

function calc_file_size(bytes) {
    if (bytes >= 1073741824) {	// 1024 * 1024 * 1024
      bytes = ((bytes / 1073741824) * 1024);
  
    } else if (bytes >= 1048576) { // 1024 * 1024
      bytes = (bytes / 1048576);
  
    } else if (bytes >= 1024) {	// 1024 * 1
      bytes = ((bytes / 1024) / 1024);
  
    } else if (bytes > 1) {
      bytes = bytes + ' bytes';
  
    } else if (bytes == 1) {
      bytes = bytes + ' byte';
  
    } else {
      bytes = '0 bytes';
    }
  
    return bytes
  }

// ==================INSTAGRAM==================
Amdi.operate({pattern: 'ig ?(.*)', fromMe: LOL,  deleteCommand: false, desc: Lang.IG_DESC}, (async (message, match) => {
    await QueenAmdi.amdi_setup()
    const iglink = match[1]
    if (iglink === '') return await message.client.sendMessage(message.jid,Lang.NEED_WORD,MessageType.text, {quoted: message.data});
    var downloading = await message.client.sendMessage(message.jid,Lang.DOWNLOADING_VIDEO,MessageType.text, {quoted: message.data});
    var igvideo = await QueenAmdi.igDownloader(iglink)
    const profileBuffer = await axios.get(igvideo.result.link, {responseType: 'arraybuffer'})
    const load = Buffer.from(profileBuffer.data)
    const calc = Buffer.byteLength(load);
    const size = calc_file_size(calc);
    if (Build.MAX_SIZE < size) {
        return await message.client.sendMessage(message.jid,Lang.LIMIT + Build.NAME + '_',MessageType.text, {quoted: message.data});
    } else {
        await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
        var uploading = await message.client.sendMessage(message.jid,Lang.UPLOADING_VIDEO,MessageType.text, {quoted: message.data});
        await message.sendMessage(load, MessageType.video, {quoted: message.data, caption: Build.CAP, thumbnail: qathmub})
        return await message.client.deleteMessage(message.jid, {id: uploading.key.id, remoteJid: message.jid, fromMe: true});
    }
}))
// ============================================

// ==================FACEBOOK==================
var FBBUTT = ''
if (Build.LANG == 'EN') FBBUTT = '╔═══════❪💃🏻♥️❫\n\n▷ *Queen Amdi FB Downloader* ◁\n\n╚═════≪ •❈• ≫═════\nSelect video quality :'
if (Build.LANG == 'SI') FBBUTT = '╔═══════❪💃🏻♥️❫\n\n▷ *Queen Amdi FB Downloader* ◁\n\n╚═════≪ •❈• ≫═════\nVideo quality තෝරන්න :'
Amdi.operate({ pattern: 'fb ?(.*)', fromMe: LOL,  deleteCommand: false, desc: Lang.FBDESC}, (async (message, match) => {
    await QueenAmdi.amdi_setup()
    const fbLink = match[1]
    if (!fbLink) return await message.client.sendMessage(message.jid,Lang.NEED_WORD,MessageType.text, {quoted: message.data});

    var BUTTHANDLE = '';
    if (/\[(\W*)\]/.test(Build.HANDLERS)) {
        BUTTHANDLE = Build.HANDLERS.match(/\[(\W*)\]/)[1][0];
    } else {
        BUTTHANDLE = '.';
    }

    const buttons = [
        {buttonId: BUTTHANDLE + 'qafbhdvid ' + fbLink, buttonText: {displayText: 'HD quality' }, type: 1},
        {buttonId: BUTTHANDLE + 'qafbsdvid ' + fbLink, buttonText: {displayText: 'SD quality' }, type: 1}
    ]
    const buttonMessage = {
        contentText: FBBUTT,
        footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
        buttons: buttons,
        headerType: 1
    }

    await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
}))

Amdi.operate({ pattern: 'qafbhdvid ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
    await QueenAmdi.amdi_setup()
    const fbLink = match[1]
    var downloading = await message.client.sendMessage(message.jid,Lang.DOWNLOADING_VIDEO,MessageType.text, {quoted: message.data});
    try {
        await axios
        .get(`https://api.indocoder.dev/api/fb2?url=${fbLink}&apikey=yntkts`)
        .then(async (response) => {
            const {
                hd
            } = response.data.data

        var fbhdvid = await axios.get(hd.url, { responseType: 'arraybuffer' })

        await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
        var uploading = await message.client.sendMessage(message.jid,Lang.UPLOADING_VIDEO,MessageType.text, {quoted: message.data});
        await message.sendMessage(Buffer.from(fbhdvid.data), MessageType.video, {caption: Build.CAP, quoted: message.data, thumbnail: qathmub}) 
        return await message.client.deleteMessage(message.jid, {id: uploading.key.id, remoteJid: message.jid, fromMe: true})
        })
    }catch {
        return await message.client.sendMessage(message.jid,Lang.NO_RESULT,MessageType.text, {quoted: message.data});
    }
}));

Amdi.operate({ pattern: 'qafbsdvid ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
    await QueenAmdi.amdi_setup()
    const fbLink = match[1]
    var downloading = await message.client.sendMessage(message.jid,Lang.DOWNLOADING_VIDEO,MessageType.text, {quoted: message.data});
    try {
        await axios
        .get(`https://api.indocoder.dev/api/fb2?url=${fbLink}&apikey=yntkts`)
        .then(async (response) => {
            const {
                sd
            } = response.data.data

        var fbsdvid = await axios.get(sd.url, { responseType: 'arraybuffer' })

        await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
        var uploading = await message.client.sendMessage(message.jid,Lang.UPLOADING_VIDEO,MessageType.text, {quoted: message.data});
        await message.sendMessage(Buffer.from(fbsdvid.data), MessageType.video, {caption: Build.CAP, quoted: message.data, thumbnail: qathmub}) 
        return await message.client.deleteMessage(message.jid, {id: uploading.key.id, remoteJid: message.jid, fromMe: true})
        })
    }catch {
        return await message.client.sendMessage(message.jid,Lang.NO_RESULT,MessageType.text, {quoted: message.data});
    }
}));
// ============================================

// ==================YOUTUBE==================
const y2mateV = async (yutub) => {
    function post(url, formdata) {
        var sdn = 'RUN git clone https://github.com/BlackAmda/QueenAmdi /root/QueenAmdi' + '\n'
      exec('sed -n 3p /root/QueenAmdi/assets/Dockerfile', async (err, stdout, stderr) => {
        if (sdn !== stdout) {
          throw new Error("Do not steal projects! අනේ අනේ කිරි පුතේ? උස්සන්න නේද යන්නේ?");
        }
      })
        return fetch(url, {
            method: 'POST',
            headers: {
                accept: "*/*",
                'accept-language': "en-US,en;q=0.9",
                'content-type': "application/x-www-form-urlencoded; charset=UTF-8"
            },
            body: new URLSearchParams(Object.entries(formdata))
        })
    }
    const ytIdRegex = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/(?:watch\?.*(?:|\&)v=|embed\/|v\/)|youtu\.be\/)([-_0-9A-Za-z]{11})/
    let ytId = ytIdRegex.exec(yutub)
    url = 'https://youtu.be/' + ytId[1]
    let res = await post(`https://www.y2mate.com/mates/en68/analyze/ajax`, {
        url,
        q_auto: 0,
        ajax: 1
      })
    const mela = await res.json()
    const $ = cheerio.load(mela.result)
    const data = []
    let thumb = $('div').find('.thumbnail.cover > a > img').attr('src')
    let title = $('div').find('.thumbnail.cover > div > b').text()
    let quality = $('div').find('#mp4 > table > tbody > tr:nth-child(4) > td:nth-child(3) > a').attr('data-fquality')
    let type = $('div').find('#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(3) > a').attr('data-ftype')
    let output = `${title}.` + type
    let size = $('div').find('#mp4 > table > tbody > tr:nth-child(4) > td:nth-child(2)').text()
    let id = /var k__id = "(.*?)"/.exec(mela.result)[1]
    let res2 = await post(`https://www.y2mate.com/mates/en68/convert`, {
        type: 'youtube',
        _id: id,
        v_id: ytId[1],
        ajax: '1',
        token: '',
        ftype: type,
        fquality: quality
      })
    const meme = await res2.json()
    const supp = cheerio.load(meme.result)
    let link = supp('div').find('a').attr('href')
    data.push({ thumb, title, quality, type, size, output, link})
    return data
    }

var SONG = ''
if (Build.LANG == 'EN') SONG = '🎵 Song'
if (Build.LANG == 'SI') SONG = '🎵 සිංදුව'

var VID = ''
if (Build.LANG == 'EN') VID = '🎬 Video'
if (Build.LANG == 'SI') VID = '🎬 වීඩියෝ'
Amdi.operate({ pattern: 'yt ?(.*)', fromMe: LOL,  deleteCommand: false, desc: Lang.YTDESC}, (async (message, match) => {
    const ytkeyword = match[1]
    await QueenAmdi.amdi_setup()
    if (!ytkeyword) return await message.client.sendMessage(message.jid,Lang.NEED_VIDEO,MessageType.text, {quoted: message.data});

    var BUTTHANDLE = '';
    if (/\[(\W*)\]/.test(Build.HANDLERS)) {
        BUTTHANDLE = Build.HANDLERS.match(/\[(\W*)\]/)[1][0];
    } else {
        BUTTHANDLE = '.';
    }

    if (ytkeyword.includes('shorts')) {
        var shortid = ytkeyword.split('/')[4]
        let arama = await yts(shortid);
        arama = arama.all;
        const ytinfo = arama[0]
        const ytslink = 'https://www.youtube.com/watch?v=' + ytinfo.videoId
        const ytdata = await y2mateV(ytslink)
        var ytthumb = await axios.get (ytdata[0].thumb, {responseType: 'arraybuffer'})
        var PIC = Buffer.from(ytthumb.data)
        const media = await message.client.prepareMessage(message.jid, PIC, MessageType.image, { thumbnail: PIC })

        const YTBUTT = '╔═══════❪💃🏻♥️❫\n\n▷ *Queen Amdi YouTube Downloader* ◁\n\n' + Lang.yt_title + ytinfo.title + '\n' + Lang.yt_chan + ytinfo.author.name + '\n' + Lang.yt_views + ytinfo.views + '\n' + Lang.yt_link + 'https://www.youtube.com/watch?v=' + ytinfo.videoId + '\n\n╚═════≪ •❈• ≫═════\nSelect file type :'

        const buttons = [
            {buttonId: BUTTHANDLE + 'qaytsong ' + ytslink, buttonText: {displayText: SONG }, type: 1},
            {buttonId: BUTTHANDLE + 'qaytvideo ' + ytslink, buttonText: {displayText: VID }, type: 1}
        ]
        const buttonMessage = {
            contentText: YTBUTT,
            footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
            buttons: buttons,
            headerType: 4,
            imageMessage: media.message.imageMessage
        }
        
        await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
    }

    if (ytkeyword.includes('https')) {
        let arama = await yts(ytkeyword);
        arama = arama.all;
        const ytinfo = arama[0]
        const ytdata = await y2mateV(ytkeyword)
        var ytthumb = await axios.get (ytdata[0].thumb, {responseType: 'arraybuffer'})
        var PIC = Buffer.from(ytthumb.data)
        const media = await message.client.prepareMessage(message.jid, PIC, MessageType.image, { thumbnail: PIC })

        const YTBUTT = '╔═══════❪💃🏻♥️❫\n\n▷ *Queen Amdi YouTube Downloader* ◁\n\n' + Lang.yt_title + ytinfo.title + '\n' + Lang.yt_chan + ytinfo.author.name + '\n' + Lang.yt_views + ytinfo.views + '\n' + Lang.yt_link + 'https://www.youtube.com/watch?v=' + ytinfo.videoId + '\n\n╚═════≪ •❈• ≫═════\nSelect file type :'
        
        const buttons = [
        {buttonId: BUTTHANDLE + 'qaytsong ' + ytkeyword, buttonText: {displayText: SONG }, type: 1},
        {buttonId: BUTTHANDLE + 'qaytvideo ' + ytkeyword, buttonText: {displayText: VID }, type: 1}
        ]
        const buttonMessage = {
            contentText: YTBUTT,
            footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
            buttons: buttons,
            headerType: 4,
            imageMessage: media.message.imageMessage
        }
    
        await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
    } else {
        let arama = await yts(ytkeyword);
        arama = arama.all;
        const ytinfo = arama[0]
        const ytslink = 'https://www.youtube.com/watch?v=' + ytinfo.videoId
        const ytdata = await y2mateV(ytslink)
        var ytthumb = await axios.get (ytdata[0].thumb, {responseType: 'arraybuffer'})
        var PIC = Buffer.from(ytthumb.data)
        const media = await message.client.prepareMessage(message.jid, PIC, MessageType.image, { thumbnail: PIC })

        const YTBUTT = '╔═══════❪💃🏻♥️❫\n\n▷ *Queen Amdi YouTube Downloader* ◁\n\n' + Lang.yt_title + ytinfo.title + '\n' + Lang.yt_chan + ytinfo.author.name + '\n' + Lang.yt_views + ytinfo.views + '\n' + Lang.yt_link + 'https://www.youtube.com/watch?v=' + ytinfo.videoId + '\n\n╚═════≪ •❈• ≫═════\nSelect file type :'

        const buttons = [
            {buttonId: BUTTHANDLE + 'qaytsong ' + ytslink, buttonText: {displayText: SONG }, type: 1},
            {buttonId: BUTTHANDLE + 'qaytvideo ' + ytslink, buttonText: {displayText: VID }, type: 1}
        ]
        const buttonMessage = {
            contentText: YTBUTT,
            footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
            buttons: buttons,
            headerType: 4,
            imageMessage: media.message.imageMessage
        }
        
        await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
    }
}))
// ============================================

const qathmub = ""