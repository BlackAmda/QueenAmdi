/*
░██████╗░██╗░░░██╗███████╗███████╗███╗░░██╗
██╔═══██╗██║░░░██║██╔════╝██╔════╝████╗░██║
██║██╗██║██║░░░██║█████╗░░█████╗░░██╔██╗██║
╚██████╔╝██║░░░██║██╔══╝░░██╔══╝░░██║╚████║
░╚═██╔═╝░╚██████╔╝███████╗███████╗██║░╚███║
░░░╚═╝░░░░╚═════╝░╚══════╝╚══════╝╚═╝░░╚══╝
░█████╗░███╗░░░███╗██████╗░██╗
██╔══██╗████╗░████║██╔══██╗██║
███████║██╔████╔██║██║░░██║██║
██╔══██║██║╚██╔╝██║██║░░██║██║ █▀█ █▀▀█ █▀█ ▄█─ 
██║░░██║██║░╚═╝░██║██████╔╝██║ ─▄▀ █▄▀█ ─▄▀ ─█─ 
╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═════╝░╚═╝ █▄▄ █▄▄█ █▄▄ ▄█▄
Copyright (C) 2021 Black Amda.
Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.
*/

//Basic requirements
const QueenAmdi = require('queenamdi-public');
const Amdi = QueenAmdi.events

const {MessageType,Mimetype} = require('@blackamda/queenamdi-web-api');
const got = require('got');
const axios = require('axios')

// Build Checker
const Build = QueenAmdi.build
let LOL = Build.WORKTYPE == 'public' ? false : true

//Language
const Language = require('../language');
const Lang = Language.getString('unlimiteddata');

var EHIBUT = ''
if (Build.LANG == 'SI') EHIBUT = 'ℹ️ EHI ගොනුව යනු කුමක්ද?'
if (Build.LANG == 'EN') EHIBUT = 'ℹ️ What is EHI file?'

var EHIHELP = ''
if (Build.LANG == 'SI') EHIHELP = '📄 උපදෙස්'
if (Build.LANG == 'EN') EHIHELP = '📄 Instructions'

var EHIINS = ''
if (Build.LANG == 'SI') EHIINS = '📂 අවශ්‍ය ගොනු බාගන්න'
if (Build.LANG == 'EN') EHIINS = '📂 Download files'

Amdi.operate({pattern: 'ehi', fromMe: LOL, desc: Lang.ehi_desc,  deleteCommand: false}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    const buttons = [
        {buttonId: '.qaehiinfo', buttonText: {displayText: EHIBUT }, type: 1},
        {buttonId: '.qaehihelp', buttonText: {displayText: EHIHELP }, type: 1},
        {buttonId: '.qaehidown', buttonText: {displayText: EHIINS }, type: 1}
    ]
    const buttonMessage = {
        contentText: Lang.ehi_help,
        footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
        buttons: buttons,
        headerType: 1
    }

    await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
});


// ==============EHI FUNCTIONS======================
Amdi.operate({pattern: 'qaehiinfo', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    var ehilogo = await axios.get(`https://i.ibb.co/HxZC8Xg/HTTPINJ-LOGO.png`, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid,Buffer.from(ehilogo.data), MessageType.image, {mimetype: Mimetype.jpg, caption: Lang.ehi_info, quoted: message.data, thumbnail: qathmub  });
});

Amdi.operate({pattern: 'qaehihelp', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    var help = await axios.get(`https://i.ibb.co/Qj1Km5R/help-icon.png`, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid,Buffer.from(help.data), MessageType.image, {mimetype: Mimetype.jpg, caption: Lang.ehi_how, quoted: message.data, thumbnail: qathmub  });
});

var FILEDESC = ''
if (Build.LANG == 'SI') FILEDESC = '🔽 ඔබට අවශ්‍ය files මෙතැනින් බාගන්න,'
if (Build.LANG == 'EN') FILEDESC = '🔽 Download the files you need here,'

Amdi.operate({pattern: 'qaehidown', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    const buttons = [
        {buttonId: '.qahttpapp', buttonText: {displayText: '🧰 HTTP Injector' }, type: 1},
        {buttonId: '.qaallehi', buttonText: {displayText: '💾 All EHI files' }, type: 1},
    ]
    const buttonMessage = {
        contentText: FILEDESC,
        footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
        buttons: buttons,
        headerType: 1
    }

    await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
})

Amdi.operate({pattern: 'qahttpapp', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    let url = `https://gist.github.com/BlackAmda/38678020d5077578876c92177d4f8090/raw/`
    const response = await got(url);
	const path = JSON.parse(response.body);
    const apk = await axios.get(path.app.link, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid,Buffer.from(apk.data), MessageType.document, {filename: 'HTTP_Injector.apk', quoted: message.data, mimetype: 'application/vnd.android.package-archive'});
});

var DOWNDESC = ''
if (Build.LANG == 'SI') DOWNDESC = '💠 *Queen Amdi EHI files බාගැනීම්*\n\n```EHI පිටුවක් තෝරන්න :```\n'
if (Build.LANG == 'EN') DOWNDESC = '💠 *Queen Amdi EHI files Downloader*\n\n```Select a EHI page :```\n'

Amdi.operate({pattern: 'qaallehi', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    const buttons = [
        {buttonId: '.qaehipgone', buttonText: {displayText: 'Page 01' }, type: 1},
        {buttonId: '.qaehipgtwo', buttonText: {displayText: 'Page 02' }, type: 1},
        {buttonId: '.qaehipgthree', buttonText: {displayText: 'Page 03' }, type: 1}
    ]
    const buttonMessage = {
        contentText: DOWNDESC,
        footerText: 'ǫᴜᴇᴇɴ ᴀᴍᴅɪ © 🇵‌🇺‌🇧‌🇱‌🇮‌🇨‌ ᴇᴅɪᴛɪᴏɴ',
        buttons: buttons,
        headerType: 1
    }
    await message.client.sendMessage(message.jid, buttonMessage, MessageType.buttonsMessage, {quoted: message.data});
})

Amdi.operate({pattern: 'qaehipgone', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    let url = `https://gist.github.com/BlackAmda/38678020d5077578876c92177d4f8090/raw/`
    const response = await got(url);
	const path = JSON.parse(response.body);
    const one = path.pageone
    const file = path.nameone
    const dwone = await axios.get(one.one, { responseType: 'arraybuffer' })
    const dwtwo = await axios.get(one.two, { responseType: 'arraybuffer' })
    const dwthree = await axios.get(one.three, { responseType: 'arraybuffer' })
    const dwfour = await axios.get(one.four, { responseType: 'arraybuffer' })
    const dwfive = await axios.get(one.five, { responseType: 'arraybuffer' })
    const dwsix = await axios.get(one.six, { responseType: 'arraybuffer' })
    const dwseven = await axios.get(one.seven, { responseType: 'arraybuffer' })
    const dweight = await axios.get(one.eight, { responseType: 'arraybuffer' })
    const dwnine = await axios.get(one.nine, { responseType: 'arraybuffer' })
    const dwten = await axios.get(one.ten, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid, path.textone, MessageType.text, {quoted: message.data});
    await message.client.sendMessage(message.jid,Buffer.from(dwone.data), MessageType.document, {filename: file.fileone, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwtwo.data), MessageType.document, {filename: file.filetwo, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwthree.data), MessageType.document, {filename: file.filethree, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfour.data), MessageType.document, {filename: file.filefour, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfive.data), MessageType.document, {filename: file.filefive, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwsix.data), MessageType.document, {filename: file.filesix, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwseven.data), MessageType.document, {filename: file.fileseven, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dweight.data), MessageType.document, {filename: file.fileeight, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwnine.data), MessageType.document, {filename: file.filenine, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwten.data), MessageType.document, {filename: file.fileten, quoted: message.data, mimetype: 'application/octet-stream'});
})

Amdi.operate({pattern: 'qaehipgtwo', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    let url = `https://gist.github.com/BlackAmda/38678020d5077578876c92177d4f8090/raw/`
    const response = await got(url);
	const path = JSON.parse(response.body);
    const one = path.pagetwo
    const file = path.nametwo
    const dwone = await axios.get(one.one, { responseType: 'arraybuffer' })
    const dwtwo = await axios.get(one.two, { responseType: 'arraybuffer' })
    const dwthree = await axios.get(one.three, { responseType: 'arraybuffer' })
    const dwfour = await axios.get(one.four, { responseType: 'arraybuffer' })
    const dwfive = await axios.get(one.five, { responseType: 'arraybuffer' })
    const dwsix = await axios.get(one.six, { responseType: 'arraybuffer' })
    const dwseven = await axios.get(one.seven, { responseType: 'arraybuffer' })
    const dweight = await axios.get(one.eight, { responseType: 'arraybuffer' })
    const dwnine = await axios.get(one.nine, { responseType: 'arraybuffer' })
    const dwten = await axios.get(one.ten, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid, path.texttwo, MessageType.text, {quoted: message.data});
    await message.client.sendMessage(message.jid,Buffer.from(dwone.data), MessageType.document, {filename: file.fileone, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwtwo.data), MessageType.document, {filename: file.filetwo, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwthree.data), MessageType.document, {filename: file.filethree, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfour.data), MessageType.document, {filename: file.filefour, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfive.data), MessageType.document, {filename: file.filefive, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwsix.data), MessageType.document, {filename: file.filesix, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwseven.data), MessageType.document, {filename: file.fileseven, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dweight.data), MessageType.document, {filename: file.fileeight, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwnine.data), MessageType.document, {filename: file.filenine, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwten.data), MessageType.document, {filename: file.fileten, quoted: message.data, mimetype: 'application/octet-stream'});
})

Amdi.operate({pattern: 'qaehipgthree', fromMe: LOL, deleteCommand: false, dontAddCommandList: true}, async (message, match) => {
    await QueenAmdi.amdi_setup()
    let url = `https://gist.github.com/BlackAmda/38678020d5077578876c92177d4f8090/raw/`
    const response = await got(url);
	const path = JSON.parse(response.body);
    const one = path.pagethree
    const file = path.namethree
    const dwone = await axios.get(one.one, { responseType: 'arraybuffer' })
    const dwtwo = await axios.get(one.two, { responseType: 'arraybuffer' })
    const dwthree = await axios.get(one.three, { responseType: 'arraybuffer' })
    const dwfour = await axios.get(one.four, { responseType: 'arraybuffer' })
    const dwfive = await axios.get(one.five, { responseType: 'arraybuffer' })
    const dwsix = await axios.get(one.six, { responseType: 'arraybuffer' })
    const dwseven = await axios.get(one.seven, { responseType: 'arraybuffer' })
    const dweight = await axios.get(one.eight, { responseType: 'arraybuffer' })
    const dwnine = await axios.get(one.nine, { responseType: 'arraybuffer' })
    const dwten = await axios.get(one.ten, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid, path.textthree, MessageType.text, {quoted: message.data});
    await message.client.sendMessage(message.jid,Buffer.from(dwone.data), MessageType.document, {filename: file.fileone, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwtwo.data), MessageType.document, {filename: file.filetwo, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwthree.data), MessageType.document, {filename: file.filethree, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfour.data), MessageType.document, {filename: file.filefour, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwfive.data), MessageType.document, {filename: file.filefive, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwsix.data), MessageType.document, {filename: file.filesix, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwseven.data), MessageType.document, {filename: file.fileseven, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dweight.data), MessageType.document, {filename: file.fileeight, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwnine.data), MessageType.document, {filename: file.filenine, quoted: message.data, mimetype: 'application/octet-stream'});
    await message.client.sendMessage(message.jid,Buffer.from(dwten.data), MessageType.document, {filename: file.fileten, quoted: message.data, mimetype: 'application/octet-stream'});
})


const qathmub = ""